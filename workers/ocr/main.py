...
Polls Supabase for queued OCR jobs, processes them using DeepSeek-OCR,
...
from workers.ocr.ocr_engine import run_ocr, run_ocr_with_tiling, run_ocr_adaptive, build_ocr_output
...
def process_job(db: SupabaseDB, r2: R2Client, job: dict) -> None:
    ...
    try:
        logger.info(f"[job={job_id}] Using DeepSeek-OCR adaptive mode")
...
def run_daemon(poll_seconds: int) -> None:
    ...
    logger.info("Initializing DeepSeek-OCR engine (this may take a moment)...")
    from workers.ocr.ocr_engine import get_deepseek_model
    get_deepseek_model()
    logger.info("DeepSeek-OCR engine ready")
